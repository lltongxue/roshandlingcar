...
void getpathposition(const geometry_msgs::Point msg)
{
    path_x = msg.x;
    path_y = msg.y;
    path_xita = msg.z;
    //cout<<"path1:"<<msg.x<<endl;
    //cout<<"path2:"<<msg.y<<endl;
    //ROS_INFO("I heard");
}
...
void control_algorithm()
{
    ...
    //速度和角速度控制
    if (L > 1 && control_speed<=0.5)
        control_speed = control_speed + 0.0002;
    else if(L <= 1 && L>=0.05)
        control_speed = L*0.5; 
    else if(L < 0.05)
        control_speed = 0;  
...
int main(int argc, char **argv)
{
    ...
    ros::Subscriber subpath = n.subscribe("ref_path_topic", 1000, getpathposition);
